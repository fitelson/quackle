...
- Multiplayer uses GKTurnBasedMatch with a hard-coded two-account allowlist (`FamilyMultiplayer`) and a direct invite to the configured opponent (`GKMatchRequest.recipients`)
...
- Only the two configured `gamePlayerID`s may enter multiplayer; unauthorized local accounts disable Play Online, and matches/events/polls/submissions with unexpected participants are ignored or rejected
...
- bestPlayableMatch() skips unpaired empty matches (no opponent yet) during selection so find(for:) can pair/invite fresh, but does NOT remove them — removal disrupts the other device's pending request
...
- appendLatestMoveToHistory() reads each entry's running total from live bridge.score(forPlayerIndex:) matched by player name, not the bridge entry's totalScore (which only covers moves since last restore)
