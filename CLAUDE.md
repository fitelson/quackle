...
- Bingo vocabulary is a deterministic DRAW-PROBABILITY model (not a hash, not a per-turn dice roll, not a normal-CDF approximation): the bridge scores each bingo's 7 newly-laid tiles by `logCount = Σ ln C(bagCount[L], timesUsed[L])` (`bingoDrawLogCount`), then KEEPS the bingo iff `logCount >= bingoLnQuantile(1 - bingoKnowledge)` — the (1 - knowledge) quantile of the REAL bingo distribution. So the slider tracks TRUE frequency: 10% ≈ the 10% most-probable bingos (RETINAS-class racks first, rare-tile racks like QUICKLY last). `kBingoQuantile[21]` is the empirical 5%-spaced percentile curve of `lnDrawCount` over the 20,362 drawable 7-letter words in `/usr/share/dict/words` (CSW19 proxy; regenerate with `tools/bingo_calib.py`). Sanity: REGIONS (ln 12.83) → ~98th percentile (top ~2%), so it's known at the 5% slider; matches its real-world top-2% bingo rank. Non-bingo moves always stay; falls back to all moves if the filter empties the pool. Top-100 candidate pool still applies
