...
- Bingo vocabulary is a deterministic DRAW-PROBABILITY model (not a hash, not a per-turn dice roll): the bridge scores each bingo's 7 newly-laid tiles by `logCount = Σ ln C(bagCount[L], timesUsed[L])` (`bingoDrawLogCount`), maps it to a percentile via a normal CDF (`0.5*erfc(-z/√2)`, z from `kBingoLogMean=10.5`/`kBingoLogStd=1.6`), and KEEPS the bingo only if its percentile ≥ `1 - bingoKnowledge` — i.e. the AI knows the top `bingoKnowledge` fraction of bingos BY COMMONNESS (RETINAS-class racks first, rare-tile racks last). Non-bingo moves always stay; falls back to all moves if the filter empties the pool. `kBingoLogMean/Std` are rough runtime-uncalibrated constants (no lexicon distribution available) — tune to shift slider feel. Top-100 candidate pool still applies
