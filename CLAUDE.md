...
- EITHER-CAN-START matchmaking convergence (fixes the original "three separate games" bug; lets either player initiate). `FamilyMultiplayer.ownerID = allowedIDs.min()` and `localIsOwner` are computed purely from the local gamePlayerID (zero coordination). EITHER player can initiate a game; the "owner" is only a deterministic tiebreak KEEPER. In `findOrCreateMatch`, after KVS + `bestPlayableMatch` find no shared game: the OWNER reuses its single canonical open seat via `ownerCanonicalSeat()` (smallest-matchID own unpaired/no-data seat, removing extras — so repeated taps + the owner's own multiple devices converge), else `find(for:)`; the NON-OWNER calls `removeOwnUnpairedMatches()` then `find(for:)`. `handleMatchFound` initializes a fresh game whenever `isMyTurn` (whoever created the seat goes first; a joiner waits). CONVERGENCE IS NON-DESTRUCTIVE: the common cases (one starts → other joins later; or one taps slightly first) are handled by `find(for:)` JOINING the opponent's open seat. There is deliberately NO after-the-fact "yield"/seat-deletion: a non-owner can't even see the owner's unpaired seat (not a participant), and deleting a live seat would wipe an in-progress opening move (a real bug an adversarial review caught — match data only lands when the human commits the first move, so an unpaired-no-data seat is the NORMAL solo-start state, not a race signal). A true sub-second simultaneous double-create that also slips past GameKit's own dedup is rare and self-heals on the next tap (find joins the other's seat, or the owner reuses its canonical seat).
...
- Move-history descriptions in the bridge's `moveHistory` method: an exchange shows "Exchanged N tiles" (never the tiles — hidden info), a PASS shows "Pass" (NOT `move.toString()`'s "-"), everything else is `move.toString()`. (The kibitz/top-moves path special-cases these too.)
...
- PLAY-OUT SAFETY NET (`bridge.finalizeIfPlayedOut`, called at the top of `refreshState`): a player whose turn it is but whose rack is EMPTY with an EMPTY bag has played out → the game is over. Quackle's `incrementTurn` normally detects this at commit time, but a played-through FINAL-tile move was observed to slip past it — the human played their last tile, the game was NOT marked over, the AI got a spurious extra pass, and the board got STUCK on the played-out player's turn (You 327/AI 325, your turn, empty rack, bag 0, gameOver=false). The net detects `!gameOver && bag().empty() && currentPlayer().rack().empty()` and finalizes via Quackle's own `adjustScoresToFinishGame()` (stages an `UnusedTilesBonus` = +2× the opponent's remaining tiles for the out-player) + `setGameOver(true)`. Idempotent; heals an already-stuck save on relaunch (that game correctly resolved to You 335/AI 325 = 327 + 2×V(4)). It runs before refreshState reads game-over/scores, so a play-out now ends the game immediately with no extra AI turn
...
- GAME HISTORY (finished-game archive): `GameRecord` (Codable summary + compact final-board string) and `GameHistoryStore` (@Observable, @MainActor) live in `Model/GameState.swift`; `GameHistoryView`/`GameHistoryRow`/`GameHistoryDetailView` in `ContentView.swift` (no new files → no xcodegen needed). The store persists a JSON `[GameRecord]` in iCloud KVS (key `gameHistory`, per-Apple-ID so it syncs across the user's OWN devices), capped at 200, merging external changes by id (newest date wins). The engine archives a game ONCE when it ends: `recordGameOverIfNeeded()` runs at the end of `refreshState()` — online dedups by stable matchID (`store.contains`), so the opponent ENDING the game still archives it on this device and reloads/other-devices don't duplicate; AI uses the `hasRecordedGameOver` flag (reset false on new game; set `= state.isGameOver` in `loadSavedGame` so reloading a finished AI game doesn't re-archive). Board is compact-encoded (`.`=empty, upper=tile, lower=blank). Reached via the "…" menu → "Game History" (`ActiveSheet.pastGames`). Scores use the endgame-adjusted `players` values already in the engine
